...
class Lib_Model_DbTable_IssueReturn extends Libz_Base_Model {
	...
	public function getBookIssueDate($acc_no) {
		$sql = $this->select ()->from ( self::TABLE_NAME, 'issue_date' )->where ( 'acc_no = ?', $acc_no )->where ( "return_date is NULL" );
	...
	public function issueBook($acc_no, $member_id, $remark = NULL) {
		...
		$objToday = new Zend_Date ();
		$book = new Lib_Model_DbTable_Book ();
		$data = array ('acc_no' => $acc_no, 'member_id' => $member_id, 'issue_date' => $objToday->toString ( 'YYYY-MM-dd HH:mm:ss' ) );
		if (isset ( $remark )) {
			$data ['remarks'] = $remark;
		}
		try {
	...
	public static function getIssuedDocumentCount($member_id, $documentType = NULL) {
		...
		if (isset ( $documentType )) {
			...
			$incDocType = ' and membership_limit.document_type_id = "' . $documentType . '"';
		}
		$sql = 'SELECT
				  document_type_id,
