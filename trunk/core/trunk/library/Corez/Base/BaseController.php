			...
			switch ($request->getParam ( 'oper' )) {
				case 'add' :
					...
					try {
						...
						$this->_helper->logger->err ( self::getUserId () . 'Row insertion failed :' );
				...
				case 'del' :
					...
					try {
						...
						$this->_helper->logger->err ( self::getUserId () . 'Row deletion failed :' );
						$this->_helper->logger->err ( $e->getMessage () );
				...
				case 'edit' :
					...
					try {
						...
						$this->_helper->logger->err ( self::getUserId () . 'Row updation failed :' );
						$this->_helper->logger->err ( $e->getMessage () );
						$this->_helper->logger->info ( 'Processed data :' );
						$this->_helper->logger ( $data );
						$this->_helper->logger->info ( 'Request Data :' );
				...
				default :
					...
					$this->_helper->logger->crit ( self::getUserId () . 'The param "oper" has unexpected value :' . $request->getParam ( 'oper' ) );
					return false;
			}
		} else {
			$this->_helper->logger->err ( self::getUserId () . 'Criteria to access CRUD is not fullfilled' );
