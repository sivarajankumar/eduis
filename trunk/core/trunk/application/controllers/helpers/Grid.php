...
class Core_Controller_Helper_Grid extends Zend_Controller_Action_HelperAbstract
{
    ...
    public function pagelimit ()
    {
        ...
        if ($this->_count > 0) {
            ...
            $this->_count / $this->gridparam['limit']);
        } else {
            $this->total_pages = 0;
        }
        ...
        if ($this->gridparam['page']) {
            $this->offset = $this->gridparam['limit'] * $this->gridparam['page'] -
             $this->gridparam['limit']; //don't put $gridparam['limit']*($gridparam['page']-1)
        } else {
            $this->offset = 0;
        }
    }
    ...
    public function fetchdata ($columns = NULL)
    {
        ...
        if (isset($this->gridparam['sidx']) and
         isset($this->gridparam['sord'])) {
            $orderstr = $this->gridparam['sidx'] . ' ' .
             $this->gridparam['sord'];
