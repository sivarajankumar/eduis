<?php
/**
 * The Generic Class for all Assessments 
 * @author ACEIS TEAM
 *
 */
abstract class Acad_Model_Assessment_Abstract
{
    ...
    public function getTest_info_id ()
    {
        ...
        "The object's primary key (i.e. _test_info_id) is not set.", 
        Zend_Log::ERR);
    }
    /**
     * Set department id
...
LIMIT 1';
    ...
    public function setMarks ($data)
    {
        ...
        return Zend_Db_Table::getDefaultAdapter()->update('test_marks', $bind, 
        $where);
    }
    public function getMarks ()
    {
        ...
        array('student_roll_no', 'marks_scored', 'status'))->where(
        'test_info_id = ?', self::getTest_info_id());
...
WHERE (`test_info_id` = ?)';
    ...
    public function setLock ($lock = TRUE)
    {
        ...
        return Zend_Db_Table::getDefaultAdapter()->update('test_info', $bind, 
        $where);
    }
    /**
    ...
    public function fetchSchedule ($dep,$deg,$sem,$testType,$numIds = null)
    {
        ...
        $sql = $adapterClass->getDefaultAdapter()
            ...
            ->from('test_info', $cols)
            ->joinInner('subject', 
        ' subject.subject_code = test_info.subject_code', 
        array('subject_code', 'subject_name'))
            ->where('degree_id =?', $deg)
        ...
        $fetchAll = $adapterClass->getDefaultAdapter()
            ...
            $logger->debug('No test ID returned');
        }
    }
    /**
