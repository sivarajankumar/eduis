...
class Auth_Plugin_Acl_Loader extends Zend_Controller_Plugin_Abstract {
	...
	public function preDispatch(Zend_Controller_Request_Abstract $request) {
		...
		if (substr ( $actionName, 0, 4 ) == 'fill'
			 or substr ( $actionName, 0, 3 ) == 'get' ) {
			return;
		}
	...
	protected static function initUserAcl() {
		$auth = Zend_Auth::getInstance ();
		
		if (! is_array ( $authContent = $auth->getStorage ()->read () )) {
			...
			$userRoles = self::getUserRoles ( $authContent );
			
			$authData ['identity'] = $authContent;
			$authData ['moduleRole'] = $userRoles ['moduleWise'];
			$authData ['roles'] = $userRoles ['roles'];
			
			$acl->addRole ( $authData ['identity'], $authData ['roles'] );
			$authData ['acl'] = $acl;
	...
	protected static function initAcl() {
		...
		$roleResources = $selectRes->from ( 'role_resource' )->query ( Zend_Db::FETCH_GROUP )->fetchAll ();
	...
	public static function getUserRoles($user_id) {
		...
		if ($user_id == 'anon') {
			...
			$dbRoles = $selectUserRoles->from ( 'role_resource', 'module_id' )->join ( 'user_role', '`user_role`.`role_id` = `role_resource`.`role_id`', 'role_id' )->where ( "user_id = ?", $user_id )->query ()->fetchAll ( Zend_Db::FETCH_GROUP );
	...
	public function check() {
		...
		if (isset ( $authContent ['acl'] )) {
			...
			if ($userAcl instanceof Zend_Acl) {
				...
				$reqResource = strtolower ( $request->getModuleName () . '_' . $request->getControllerName () . '_' . $request->getActionName () );
				try {
					if ($userAcl->has ( $reqResource )) {
						if ($userAcl->isAllowed ( $authContent ['identity'], $reqResource )) {
							return true;
						} else {
							throw new Zend_Exception ( 'ACL denied "' . $_SERVER ['SERVER_NAME'] . '/' . str_ireplace ( '_', '/', $reqResource ) . '" to ' . $authContent ['identity'], Zend_Log::ERR );
						}
					} else {
						throw new Zend_Exception ( 'RESOURCE "' . str_ireplace ( '_', '/', $reqResource ) . '" is not found in ACL', Zend_Log::WARN );
					}
