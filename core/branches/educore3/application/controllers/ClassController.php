    ...
    public function saveclassAction ()
    {
        ...
        switch ($format) {
            case 'html':
                $this->_helper->viewRenderer->setNoRender(false);
                $this->_helper->layout()->enableLayout();
                $this->view->assign('response', $tnp_save_status);
                break;
            case 'jsonp':
                $callback = $this->getRequest()->getParam('callback');
                echo $callback . '(' .
                 $this->_helper->json($tnp_save_status, false) . ')';
                break;
            case 'json':
                $this->_helper->json($tnp_save_status);
                break;
            case 'log':
                Zend_Registry::get('logger')->debug('No format was provided..');
                Zend_Registry::get('logger')->debug($tnp_save_status);
                break;
            default:
                ;
                break;
