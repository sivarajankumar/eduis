...
class AuthenticateController extends Zend_Controller_Action
{
    ...
    public function indexAction ()
    {
        ...
        if (isset($_COOKIE[self::AUTH_SID])) {
            ...
            $g_id = Acadz_Resource_Acl_Guest::GUEST_ID;
            $auth->authenticate($guestAdapter);
            $userInfo['identity'] = $g_id;
            $userInfo['roles'][] = 'guest';
            $remoteAcl->userInfo = $userInfo;
            $this->_helper->logger(
            'No remote cookie found. So, identifying as"' . $g_id .
    ...
    public function logoutAction ()
    {
        ...
        try {
            if (isset($_COOKIE[self::AUTH_SID])) {
                ...
                $this->_helper->logger(
                'No remote cookie found. So, not requesting AUTH_SERVER to logout.');
            }
