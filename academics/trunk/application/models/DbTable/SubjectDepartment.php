<?php
class Acad_Model_DbTable_SubjectDepartment extends Aceis_Base_Model
{
	 ...
	 */
    public static function getSemesterSubjects($department, $degree, $semester, $subjectType = NULL) {
        $sql = self::getDefaultAdapter ()->select ()
                                          ->from ( array ('sdep' => 'subject_department' ),
                                                      'subject_code')
                                          ->join ( array ('sub' => 'subject' ),
                                                      '`sub`.subject_code = `sdep`.subject_code',
                                                      'subject_name' )
     ...
     * Faculty of a class
     */
    public static function getSemesterFaculty($department, $degree, $semester, $subjectType = NULL) {
        $sql = self::getDefaultAdapter ()->select ()
                                            ->distinct()
                                            ->from ( array ('subfac' => 'subject_faculty' ),
                                                    'staff_id')
                                            ->join ( array ('subdep' => 'subject_department' ),
                                                    '`subfac`.subject_code = `subdep`.subject_code',
                                                    array() )
