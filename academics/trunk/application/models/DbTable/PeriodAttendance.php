...
class Acad_Model_DbTable_PeriodAttendance extends Acadz_Base_Model_Dbtable
{
     ...
     * @version 2.0
     */
    public static function expectedLectures ($ttids, $dateFrom = NULL, 
    $dateTo = NULL)
    {
    ...
    public function markPeriodAttendance ($period_date, $timetable_id, $staff_id, 
    $marked_date)
    {
        $set = array('marked_date' => $marked_date, 'staff_id' => $staff_id);
        $where[] = $this->getAdapter()->quoteInto('period_date = ?', 
        $period_date);
        $where[] = $this->getAdapter()->quoteInto('timetable_id = ?', 
        $timetable_id);
