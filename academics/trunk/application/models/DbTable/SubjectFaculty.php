<?php
class Acad_Model_DbTable_SubjectFaculty extends Acadz_Base_Model_Dbtable
{
...
WHERE (subject_faculty.staff_id = ?)';
	 ...
	 * Subject's Faculty
	 */
    public static function getSubjectFaculty ($subjectCode, $subjectMode = NULL, 
    $department = NULL)
    {
        $sql = self::getDefaultAdapter()->select()
            ->from(self::TABLE_NAME, 'staff_id') //'staff_id',   //Use it from either table
    ...
    public function insert (array $data)
    {
        ...
        try {
            ...
            return Zend_Controller_Front::getInstance()->getResponse()->setHttpResponseCode(
            200);
        } catch (Exception $e) {
            throw $e;
        }
    ...
    public static function getDepartmentFaculty ($department_id)
    {
        $sql = self::getDefaultAdapter()->select()
            ...
            ->join('staff_personal', 
        'subject_faculty.staff_id = staff_personal.staff_id', 
        array('first_name', 'last_name'))
            ->join('semester_degree', 
        'semester_degree.department_id = subject_faculty.department_id')
