<?php
class Acad_Model_DbTable_FacultyAdjustment extends Aceis_Base_Model {
	
	protected $_name = 'faculty_adjustment';
	...
	public function adjustperiod($data) {
		$sql = 'INSERT INTO `faculty_adjustment` (`source_timetable_id`, `start_date`, `source_staff_id`,`target_timetable_id`,`target_staff_id`) VALUES ';
	...
	$sql = self::getDefaultAdapter()->select()->from('faculty_adjustment','source_timetable_id')
	...
	public static function getAdjustment($staff_id,$period_date)
	{
      ...
      FROM (SELECT
              timetable.period_id,
              faculty_adjustment.source_timetable_id,
              ...
              faculty_adjustment.source_staff_id
            FROM timetable
              INNER JOIN period
              ...
              INNER JOIN faculty_adjustment
                ON faculty_adjustment.target_timetable_id = timetable.timetable_id
                  AND faculty_adjustment.target_staff_id = ?
                  AND faculty_adjustment.start_date = ?
  ...
  INNER JOIN period
    ON tmp_2.period_id = period.period_id";
		 $resultSet = self::getDefaultAdapter()->fetchAll($sql,array($staff_id,$period_date));
		 return $resultSet;
	}
	
	public static function cancelAdjustment($period_id,$staff_id ,$period_date) {
		
		$result = Department_Model_DbTable_Timetable::getPeriodIdTimetable($period_id,$staff_id);
		$where = "source_staff_id='$staff_id'";
		...
		$or_stmt .= ')';
		$where = $where . $or_stmt ;
		return self::getDefaultAdapter()->delete('faculty_adjustment', $where );
