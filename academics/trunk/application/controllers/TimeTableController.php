...
class Department_TimeTableController extends Aceis_Base_BaseController {
	...
	public function init() {
		$this->model = new Department_Model_DbTable_TimeTable ();
		...
		$this->department_id = $_SESSION ['staff_detail'] ['department_id'];
	...
	public function indexAction() {
		...
		$this->view->assign ( 'slaveDepartment', Model_DbTable_SemesterDegree::slaveDepartment ( $masterDepartment ) );
	}
	...
	public function fillgridAction() {
		...
		if ($valid) {
			...
			$masterDepartment = $this->department_id;
			$slaves = Model_DbTable_SemesterDegree::slaveInfo ( $masterDepartment, 'thisSessionOnly', 'showDegree', 'showSemester' );
			$where = '';
			$setOr = false;
			...
			$sql = Zend_Db_Table::getDefaultAdapter ()->select ()->from ( array ('prd' => 'period' ), array ('department_id', 'degree_id', 'semester_id', 'weekday.weekday_name as weekday_number', 'period_number' ) )->join ( array ('tt' => 'timetable' ), '`prd`.period_id = `tt`.period_id', array ('timetable_id', 'subject_code', 'subject_mode_id', 'group_id', 'CONCAT(first_name," ", last_name) as staff_id', 'period_duration', 'periods_covered', 'valid_from', 'valid_upto', 'block_id', 'room_id' ) )->join ( 'weekday', '`prd`.weekday_number = weekday.weekday_number', array ('weekday_number' ) )->join ( 'staff_personal', '`tt`.staff_id = staff_personal.staff_id', array () );
				...
				foreach ( $sarr as $key => $value ) {
					switch ($key) {
						case 'department_id' :
							...
							break;
						case 'semester_id,weekday_number,period_number' :
							$sql->where ( "weekday.weekday_name LIKE ?", $value . '%' );
							break;
						case 'period_number' :
	...
	public function isvaliddurationAction() {
		...
		$objGroups = new Model_DbTable_Groups ();
		$objPeriod = new Department_Model_DbTable_Period ();
		$request = $this->getRequest ();
		...
		$groups = $objGroups->getClassGroups ( $department_id, $degree_id );
