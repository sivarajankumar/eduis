<?php
class SubjectModeController extends Acadz_Base_BaseController
{
    /*
     * @about Interface.
     */
    ...
    public function getactivesubjectmodeAction ()
    {
        ...
        if (isset($subject_code)) {
            $result = Acad_Model_DbTable_SubjectMode::getSubjectModes(
            $subject_code, $groupMode);
            switch (strtolower($format)) {
                ...
                case 'select':
                    ...
                    foreach ($result as $key => $row) {
                        echo '<option value="' . $row['subject_mode_id'] . '">' .
                         $row['subject_mode_id'] . '</option>';
                    }
                    echo '</select>';
                    return;
            }
    ...
    public function getsubjectmodeAction ()
    {
        ...
        $subject_code = $request->getParam('subject_code');
        if (isset($subject_code)) {
            $resultSet = Acad_Model_DbTable_SubjectMode::getSubjectModes(
            $subject_code);
    ...
    public function getisgrouptogetherAction ()
    {
        ...
        if (isset($subject_mode)) {
            $result = Acad_Model_DbTable_SubjectMode::groupTogether(
            $subject_mode);
            echo $result;
            return;
        }
