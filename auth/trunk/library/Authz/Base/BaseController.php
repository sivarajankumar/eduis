 ...
 */
class Authz_Base_BaseController extends Zend_Controller_Action {
	...
	protected function createModel($forceReCreate = FALSE) {
		if ((! isset ( $this->model )) || $forceReCreate) {
			...
			$moduleName = NULL;
			//Uncomment line below if name of default module is not 'default'
			//$defaultModule = $this->getFrontController()->getDispatcher()->getDefaultModule();
			if ('default' != strtolower ( $module )) {
			...
			$model = $moduleName . 'Model_DbTable_' . $class;
